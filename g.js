...
const riftBtn = document.getElementById('rift-btn');
riftBtn.addEventListener('click', () => {
    // Stop the main game's activities
    stopGameGenesis();
    
    // Hand control over to the Rift system
    // We will define Rift.start() in the next step
    if (window.Rift && typeof window.Rift.start === 'function') {
        Rift.start();
    } else {
        console.error("Rift.js is not loaded or Rift.start is not a function.");
    }
});
