...
const firebaseConfig = {
  ...
  document.addEventListener('DOMContentLoaded', () => {
      ...
      let genesisState = {
        isActive: false, gameLoopId: null, player: null, enemies: [], lootOrbs: [],
        lastEnemySpawn: 0, difficultyLevel: 1, lastDifficultyIncrease: 0,
        isBattleMode: false, currentWave: 0, totalWaves: 20, enemiesToSpawnThisWave: 0, enemiesSpawnedThisWave: 0, boss: null, waveTransitionActive: false, totalDamageDealtThisBattle: 0,
      };
      let pvpState = { isActive: false, timerId: null, playerDamage: 0, opponentDamage: 0, opponentData: null, };
      ...
      const itemData = {
        rarities: { common: { weight: 70, budget: 1, affixes: 1 }, uncommon: { weight: 20, budget: 1.4, affixes: 2 }, rare: { weight: 7, budget: 1.9, affixes: 2 }, epic: { weight: 2.5, budget: 2.5, affixes: 3 }, legendary: { weight: 0.5, budget: 3.5, affixes: 4 } },
      ...
      const permanentShopUpgrades = {
          ...
          energyTraining: { name: "Energy Discipline", desc: "Permanently increases Max Energy by 25.", bonus: 25, levelReq: 15, maxLevel: 1000, cost: (level) => 1500 * Math.pow(2.2, level) }
      };
      const expeditionData = {
          ...
          modifiers: { gold: { name: "Rich", description: "High chance of Gold", goldMod: 1.5, itemMod: 0.8 }, item: { name: "Mysterious", description: "High chance of Items", goldMod: 0.8, itemMod: 1.5 }, xp: { name: "Dangerous", description: "High XP reward", goldMod: 1, itemMod: 1, xpMod: 1.5 }, }
      };
      const reforgeNameData = {
          prefixes: ["Forged", "Tempered", "Masterwork", "Infused", "Runed", "Shaped"],
          bases: { weapon: ["Smasher", "Edge", "Cleaver", "Point", "Ripper", "Breaker"], armor: ["Bulwark", "Aegis", "Carapace", "Wall", "Guard", "Bastion"] },
          suffixes: ["of Power", "of Doom", "of Glory", "of the Forge", "of Titans"]
      };
      const dailyRewards = [
        { day: 1, type: 'gold', amount: 500 }, { day: 2, type: 'gold', amount: 1000 }, { day: 3, type: 'consumable', id: 'storableHealthPotion', amount: 2 },
        { day: 4, type: 'gold', amount: 2500 }, { day: 5, type: 'consumable', id: 'energyPotion', amount: 5 }, { day: 6, type: 'gold', amount: 5000 }, { day: 7, type: 'item', rarity: 'rare' }
      ];
      const defaultState = {
          version: GAME_VERSION, playerName: "Guardian", tutorialCompleted: false, level: 1, xp: 0, gold: 0, healthPotions: 3, edgeStones: 0, highestBattleLevelCompleted: 0,
          stats: { strength: 5, agility: 5, fortitude: 5, stamina: 5 },
          resources: { hp: 100, maxHp: 100, energy: 100, maxEnergy: 100 },
          equipment: { weapon: null, armor: null }, inventory: [], hasEgg: false, partner: null,
          ...
          lastWeeklyRewardClaim: 0, settings: { musicVolume: 0.5, sfxVolume: 1.0, isMuted: false, isAutoBattle: false }, dojoPersonalBest: 0, pvpPersonalBest: 0, lastDailyClaim: 0, dailyStreak: 0, lastLogin: Date.now() 
      };
      ...
      const RARITY_ORDER = ['common', 'uncommon', 'rare', 'epic', 'legendary'];
            ...
            const autoBattleToggle = document.getElementById('auto-battle-checkbox');
            const autoBattleToggleContainer = document.getElementById('auto-battle-toggle-container');
            ...
            const havocRageBtn = document.getElementById('havoc-rage-btn');
            ...
            const inGameAutoBtn = document.getElementById('in-game-auto-btn');
            ...
            const thunderStrikeBtn = document.getElementById('thunder-strike-btn');
      ...
      function initMusic() { if (!musicManager.isInitialized) { musicManager.isInitialized = true; playMusic('main'); } }
      function playMusic(trackName) {
          ...
          if (oldTrackName === trackName) { if (musicManager.audio[trackName].paused) { musicManager.audio[trackName].play().catch(e => {}); } return; }
      ...
      function startBackgroundAssetLoading() { for (const key in musicFileUrls) { if (musicFileUrls[key]) { const audio = new Audio(musicFileUrls[key]); audio.loop = true; audio.preload = 'auto'; musicManager.audio[key] = audio; } } }
      ...
      async function displayTopPlayersOnMenu() {
        ...
        try {
            const query = db.collection("leaderboard").orderBy("tier", "desc").limit(10);
            const snapshot = await query.get();
            if (snapshot.empty) { loadingText.textContent = "No Legends Have Risen Yet."; return; }
            let players = snapshot.docs.map(doc => doc.data());
            players.sort((a, b) => { if (a.tier > b.tier) return -1; if (a.tier < b.tier) return 1; return b.level - a.level; });
            ...
            top3Players.forEach((data, index) => {
                ...
                if (container && sprite && nameEl) {
                    ...
                    if (data.tier >= 3) { sprite.classList.add('aura-ascended'); newAnimation = 'idle-breathe 2s ease-in-out infinite, super-aura-anim 1s ease-in-out infinite alternate'; }
                    else if (data.level >= 10) { sprite.classList.add('aura-level-10'); newAnimation = 'idle-breathe 4s ease-in-out infinite, aura-level-10-anim 2s ease-in-out infinite'; }
                    sprite.style.animation = newAnimation;
                    container.classList.add('visible');
                }
            });
            loadingText.style.display = 'none';
        } catch (error) { console.error("Error fetching top players:", error); loadingText.textContent = "Could not load leaderboard."; }
      } 
    ...
    function init() {
        ...
        auth.onAuthStateChanged(user => {
            updateAuthUI(user);
            if (user) {
                // User is signed in. Let's try to load their game.
                // loadGame() will handle all outcomes (found save, no save, etc.)
                loadGame();
            } else {
                // User is not signed in. Enable the load button ONLY if a local save exists.
                loadGameBtn.disabled = !localStorage.getItem('tapGuardianSave');
                // Show the main menu so they can choose to sign in, load local, or start new.
                showScreen('main-menu-screen');
            }
        });

        // Start passive update loops
    ...
    async function startGame() {
        // initAudio() is now called by the button click, not here.
      ...
      function rehydrateItemRarity(item) {
        ...
        if (typeof item.rarity === 'string') {
            ...
            if (rarityIndex > -1) { item.rarity = { key: rarityKey, color: item.type === 'weapon' ? itemData.weaponColors[rarityIndex] : itemData.armorColors[rarityIndex] }; }
        }
        return item;
      }
      ...
      async function migrateSaveData(loadedState) {
          if (!loadedState.version || loadedState.version < GAME_VERSION) {
              ...
              if (!loadedState.inventory) { loadedState.inventory = []; if (loadedState.equipment.weapon) loadedState.inventory.push(loadedState.equipment.weapon); if (loadedState.equipment.armor) loadedState.inventory.push(loadedState.equipment.armor); }
              ...
              if (loadedState.equipment.weapon) { rehydrateItemRarity(loadedState.equipment.weapon); }
              if (loadedState.equipment.armor) { rehydrateItemRarity(loadedState.equipment.armor); }
              if (loadedState.inventory && Array.isArray(loadedState.inventory)) { loadedState.inventory = loadedState.inventory.map(item => rehydrateItemRarity(item)); }
      ...
      async function loadGame() {
        ...
        if (auth.currentUser) {
            ...
            } catch (error) { console.error("Error loading from cloud:", error); showToast("Cloud save failed! Trying local save."); }
        }
        ...
        if (loadedState) {
            ...
            // If NO save was found, show a message and go to main menu. This PREVENTS the name prompt bug.
      ...
      async function saveGame(showToastNotification = false) {
          ...
          if (auth.currentUser) {
              ...
              } catch (error) { console.error("Error saving to cloud:", error); if (showToastNotification) showToast("Cloud save failed! Retrying locally."); localStorage.setItem('tapGuardianSave', JSON.stringify(gameState)); }
          } else {
      ...
      // --- ALL OTHER FUNCTIONS ---
      // The rest of the file from here down to the Event Listeners is correct.
      // ... (All functions like formatNumber, getXpForNextLevel, updateUI, etc.) ...
      
      // I am pasting all the functions again to be 100% sure.
      ...
      function formatNumber(num) {
        ...
        if (formattedNum.endsWith('.00')) { formattedNum = formattedNum.slice(0, -3); }
      ...
      function updateUI() {
          ...
          statKeys.forEach(key => { statColors[key] = defaultStatColor; statSourceRarityIndex[key] = -1; });
          ...
          const createStatRow = (label, value, statKey) => {
            ...
            return `<div class="stat-item"><span class="stat-label">${label}</span><span class="stat-value ${extraClass}" style="color: ${color};">${value} ${starIcon}</span></div>`;
          };
          ...
          panelHtml += '<hr class="stat-divider">';
          panelHtml += `<div class="stat-item"><span class="stat-label">Gold</span><div class="gold-rewards-row"><span class="stat-value stat-value-gold">${formatNumber(Math.floor(gameState.gold))}</span><span class="edgestone-display"><span>♦️</span><span>${(gameState.edgeStones || 0).toFixed(4)}</span></span><span class="potion-display"><span>🧪</span><span>${gameState.healthPotions || 0}</span></span><button id="rewards-btn" title="View Daily & Weekly Rewards">📅</button></div></div>`;
          ...
          if (canUseBattle) { battleBtn.textContent = `Battle (Lvl ${gameState.highestBattleLevelCompleted + 1})`; } else { battleBtn.textContent = 'Battle'; }                
          ...
          if (ascensionBtn) { if (gameState.level >= ASCENSION_LEVEL || gameState.ascension.tier > 1) { ascensionBtn.style.display = 'block'; } else { ascensionBtn.style.display = 'none'; } }
          ...
          feedBtn.disabled = onExpedition; inventoryBtn.disabled = onExpedition; shopBtn.disabled = onExpedition;
          if (onExpedition) { expeditionBtn.textContent = `On Expedition`; expeditionBtn.disabled = true; if (!expeditionInterval) { expeditionInterval = setInterval(updateExpeditionTimer, 1000); updateExpeditionTimer(); } } else { expeditionBtn.textContent = `Expedition`; expeditionBtn.disabled = false; }
      ...
      function addXP(character, amount) { 
          ...
          if (character.xp >= getXpForNextLevel(character.level)) { levelUp(character); }
          updateUI();
      }
  
      function levelUp(character) {
        const xpOver = character.xp - getXpForNextLevel(character.level);
        character.level++; character.xp = xpOver;
        character.stats.strength += 2; character.stats.agility += 2; character.stats.fortitude += 1; character.stats.stamina += 1;
        character.resources.maxHp += 10; character.resources.hp = character.resources.maxHp; character.resources.maxEnergy += 5; character.resources.energy = character.resources.maxEnergy;
        playSound('levelUp', 1, 'triangle', 440, 880);
        triggerScreenShake(400);
        if (character.isPartner) { showToast(`${character.name || 'Partner'} is now Level ${character.level}!`); }
        else {
            checkAllAchievements(); submitScoreToLeaderboard(); updateAscensionVisuals();
            let screenX, screenY;
            ...
            else { const spriteRect = characterSprite.getBoundingClientRect(); screenX = spriteRect.left + (spriteRect.width / 2); screenY = spriteRect.top; }
            createFloatingText('LEVEL UP!', screenX, screenY, { color: 'var(--accent-color)', fontSize: '2.5em', duration: 2500 });
        }
        saveGame();
      }
      ...
      function activateFrenzy() {
        ...
        if (tapCombo.currentMultiplier > 30) { createParticles({ clientX: window.innerWidth / 2, clientY: window.innerHeight / 2 }); }
        if (tapCombo.currentMultiplier >= 100) { triggerScreenFlash(); }
        updateFrenzyVisuals(); 
      ...
      function handleTap(event, isPartnerTap = false) {
          ...
          if (isPartnerTap) {
              ...
              if (!partner) return;
              if (partner.hatchTime) { partner.hatchTime -= 500; updatePartnerUI(); }
              else { if (partner.resources.energy <= 0) return; partner.resources.energy -= 0.1; createXpOrb(event, 0.5, partner); }
          } else {
              if (Math.random() < 0.005) { createXpBubble(); }
              ...
              setTimeout(() => { characterSprite.classList.remove('tapped'); updateAscensionVisuals(); }, 200); 
          }
          updateUI();
      }
      function feed() { if (gameState.gold >= 50) { gameState.gold -= 50; gameState.resources.energy = Math.min(gameState.resources.maxEnergy, gameState.resources.energy + 20); gameState.resources.hp = Math.min(gameState.resources.maxHp, gameState.resources.hp + 10); playSound('feed', 1, 'sine', 200, 600, 0.2); showNotification("Yum!", "Energy and HP restored slightly."); updateUI(); saveGame(); } else { showNotification("Not enough gold!", "You need 50 gold to feed your guardian."); } }
      ...
      function checkExpeditionStatus() {
          if (gameState.expedition.active && Date.now() > gameState.expedition.returnTime) {
              ...
              if (Math.random() < (itemFindChance * itemMod)) {
                  ...
                  if (!currentItem || foundItem.power > currentItem.power) { equipItem(foundItem); rewardText += `<br><br>New item equipped!`; }
              }
      ...
      function getTotalStat(stat) {
          ...
          if(gameState.permanentUpgrades) { for (const upgradeId in gameState.permanentUpgrades) { const upgradeData = permanentShopUpgrades[upgradeId]; if (upgradeData && upgradeData.stat === stat) { total += (gameState.permanentUpgrades[upgradeId] || 0) * upgradeData.bonus; } } }
          for (const slot in gameState.equipment) { const item = gameState.equipment[slot]; if (item && item.stats && item.stats[stat]) { total += item.stats[stat]; } }
      ...
      function ascend() {
          ...
          if (confirm(`Are you sure you want to Ascend?\n\nYour Level, Stats, Gold, and Equipment will be reset.\n\nYou will advance to World Tier ${gameState.ascension.tier + 1} and gain 1 Ascension Point. Your Perks and Permanent Shop Upgrades will remain.`)) {
              ...
              submitScoreToLeaderboard(); updateAscensionVisuals();
      ...
      async function showLeaderboard(type = 'level') {
          ...
          if(type === 'damage') { targetList = damageLeaderboardList; collectionName = 'damageLeaderboard'; orderByField = 'totalDamage'; orderByDirection = 'desc'; }
          else if (type === 'pvp') { targetList = pvpLeaderboardList; collectionName = 'pvpLeaderboard'; orderByField = 'maxDamage'; orderByDirection = 'desc'; }
          else { targetList = levelLeaderboardList; collectionName = 'leaderboard'; orderByField = 'tier'; orderByDirection = 'desc'; secondaryOrderByField = 'level'; secondaryOrderByDirection = 'desc'; }
          ...
          try {
              ...
              snapshot.forEach(doc => {
                  ...
                  let scoreText;
                  if(type === 'damage') { scoreText = `Damage: ${formatNumber(data.totalDamage)}`; }
                  else if (type === 'pvp') { scoreText = `Max Damage: ${formatNumber(data.maxDamage)}`; }
                  else { scoreText = `Level ${data.level} (Tier ${data.tier})`; }
      ...
      function equipItem(itemToEquip) { const itemIndex = gameState.inventory.findIndex(i => i && i.name === itemToEquip.name); if (itemIndex === -1) return; gameState.equipment[itemToEquip.type] = itemToEquip; updateUI(); saveGame(); }
      function refineItem(itemName) {
        const itemIndex = gameState.inventory.findIndex(i => i && i.name === itemName);
        if (itemIndex === -1) return;
        const item = gameState.inventory[itemIndex];
        if ((item.reforgeCount || 0) < 3) { showToast("Only fully reforged items (3/3) can be refined."); return; }
        const cost = 5;
        if ((gameState.edgeStones || 0) < cost) { showToast(`Not enough EdgeStones. Need ${cost}.`); return; }
        gameState.edgeStones -= cost;
        item.refineLevel = (item.refineLevel || 0) + 1;
        for (const stat in item.stats) { item.stats[stat] = Math.ceil(item.stats[stat] * 1.05); }
        ...
        updateInventoryUI(); updateUI(); saveGame();
      }
    function updateInventoryUI() {
        ...
        weaponsContainer.innerHTML = ''; armorContainer.innerHTML = '';
        ...
        const buildItemHtml = (item) => {
            let statsHtml = '';
            for (const stat in item.stats) { const value = item.stats[stat]; const suffix = (stat === 'critChance' || stat === 'goldFind') ? '%' : ''; statsHtml += `<div>+${value}${suffix} ${stat.charAt(0).toUpperCase() + stat.slice(1)}</div>`; }
            let actionButtonsHtml = ''; 
            if (currentForgeSelectionTarget !== null) { actionButtonsHtml = `<button onclick="selectItemForForge('${item.name}')">Select</button>`; }
            else {
                const isEquipped = gameState.equipment[item.type] && gameState.equipment[item.type].name === item.name;
                const equipButton = isEquipped ? '<button disabled>Equipped</button>' : `<button onclick="equipItemByName('${item.name}')">Equip</button>`;
                ...
                let refineButton = '';
                if ((item.reforgeCount || 0) >= 3) { refineButton = `<button class="refine-button" onclick="refineItem('${item.name}')">Refine (5 ♦️)</button>`; }
            ...
            if (item.refineLevel > 0) { itemColor = 'var(--xp-color)'; }
            const reforgeOrRefineText = item.refineLevel > 0 ? `Refined: +${item.refineLevel}` : `Reforged: ${item.reforgeCount || 0}/3`;
            return `<div class="inventory-item"><div class="inventory-item-info"><strong style="color:${itemColor}">${item.name}</strong><div class="item-stats">${statsHtml}${reforgeOrRefineText}</div></div>${actionButtonsHtml}</div>`;
        };
    ...
    function equipItemByName(itemName) { const itemToEquip = gameState.inventory.find(i => i.name === itemName); if (itemToEquip) { equipItem(itemToEquip); updateInventoryUI(); } }
    function selectItemForForge(itemName) {
        ...
        if (otherItem && otherItem.name === item.name) { showToast("Item is already in the other slot."); return; }
        if (otherItem && otherItem.type !== item.type) { showToast("Items must be the same type (weapon/armor)."); return; }
    ...
    // ... all other functions from generateAndShowExpeditions down to the event listeners are here ...
    // ... and are assumed to be correct based on the provided file.
    // The following is the rest of your file, with duplicate functions removed.
      ...
      function generateAndShowExpeditions() {
          ...
          for (let i = 0; i < 5; i++) {
              ...
              const expedition = { name: `${action} ${location}`, description: modifier.description, duration: duration, modifiers: { goldMod: modifier.goldMod ?? 1, itemMod: modifier.itemMod ?? 1, xpMod: modifier.xpMod ?? 1 }, index: i };
      ...
      function triggerScreenFlash() { const flashOverlay = document.getElementById('screen-flash-overlay'); flashOverlay.classList.add('flash'); setTimeout(() => { flashOverlay.classList.remove('flash'); }, 300); }
      function createXpBubble() {
        ...
        bubbleEl.onclick = () => {
            ...
            const reward = Math.floor(getXpForNextLevel(gameState.level) * (Math.random() * 0.15 + 0.05));
        ...
        setTimeout(() => { if (bubbleEl && !bubbleEl.classList.contains('popped')) { bubbleEl.remove(); } }, parseFloat(bubbleEl.style.animationDuration) * 1000);
      }
      function createFloatingText(text, x, y, options = {}) {
        ...
        textEl.style.left = `${x}px`; textEl.style.top = `${y}px`; textEl.style.color = color; textEl.style.fontSize = fontSize;
      ...
      function manualAbility_HavocRage() {
        // If the button is already "blacked out", do nothing.
        if (havocRageBtn.classList.contains('on-cooldown')) {
            return;
        }
        // Fire the ability.
        handleGenesisHavocRage(Date.now());
        // Immediately black out the button.
        havocRageBtn.classList.add('on-cooldown');
        // Set a timer to "light it up" again after 3 seconds.
        setTimeout(() => {
            havocRageBtn.classList.remove('on-cooldown');
        }, 3000);
    }

    function manualAbility_ThunderStrike() {
        if (thunderStrikeBtn.classList.contains('on-cooldown')) {
            return;
        }
        // The proximity check must remain here for the manual click to feel right.
        const thunderStrikeProximity = 50;
        let isEnemyClose = false;
        for (const enemy of genesisState.enemies) { const dx = genesisState.player.x - enemy.x; const dy = genesisState.player.y - enemy.y; if (Math.sqrt(dx * dx + dy * dy) <= thunderStrikeProximity) { isEnemyClose = true; break; } }
        if (!isEnemyClose) {
            showToast("No enemy in range for Thunder Strike!");
            return;
        }
        
        handleGenesisThunderStrike(Date.now());
        thunderStrikeBtn.classList.add('on-cooldown');
        setTimeout(() => {
            thunderStrikeBtn.classList.remove('on-cooldown');
        }, 3000);
    }

    function manualAbility_Dash() {
        if (dashBtn.classList.contains('on-cooldown')) {
            return;
        }
        initiateGenesisPlayerDash(Date.now());
    ...
    // This function runs every frame in the gameLoop to update cooldowns over time.
    function updateManualControls(timestamp) {
        if (gameState.settings.isAutoBattle || !genesisState.player) { return; }
        
        updateCooldownVisual(havocRageBtn, timestamp, genesisState.player.lastHavocRageTime, genesisState.player.havocRageCooldown);
        updateCooldownVisual(thunderStrikeBtn, timestamp, genesisState.player.lastThunderStrikeTime, genesisState.player.thunderStrikeCooldown);
        updateCooldownVisual(dashBtn, timestamp, genesisState.player.lastDashTime, genesisState.player.dashCooldown);
    }
    
    // This is the core helper function that handles the button's visual state.
    function updateCooldownVisual(button, timestamp, lastUseTime, cooldown) {
        const timeSinceUse = timestamp - lastUseTime;
        ...
        if (timeSinceUse < cooldown) {
            ...
            let overlay = button.querySelector('.ability-cooldown-overlay');
            if (!overlay) {
                overlay = document.createElement('div');
                overlay.className = 'ability-cooldown-overlay';
                button.appendChild(overlay);
            }
            const timeLeft = (cooldown - timeSinceUse) / 1000;
            const percentage = (timeSinceUse / cooldown) * 100;
            overlay.style.height = `${100 - percentage}%`;
            overlay.textContent = timeLeft.toFixed(1);

            // Set a flag so we know it's being timed.
            button.isCoolingDown = true; 

        } else if (button.isCoolingDown) { // Notice the "else if"
            // --- THIS IS THE FIX ---
            // The cooldown has JUST finished.
    ...
    function showGenesisUI() {
        ...
        autoBattleToggleContainer.style.display = 'none';
        manualControlsContainer.style.display = 'flex';
        uiToggleContainer.style.display = 'block';
        uiToggleBtn.classList.remove('active');
        updateAutoBattleUI();
    }
    function showMainMenuUI() {
        actionButtons.style.display = 'grid';
        autoBattleToggleContainer.style.display = 'flex';
        manualControlsContainer.style.display = 'none';
        uiToggleContainer.style.display = 'none';
    }
    function unlockAchievement(id) {
          ...
          if (ach.reward) {
              switch (ach.reward.type) {
                  case 'gold': gameState.gold += ach.reward.amount; rewardText = ` (+${ach.reward.amount} Gold)`; break;
                  case 'item': const newItem = generateItem(ach.reward.rarity); gameState.inventory.push(newItem); rewardText = ` (Received ${newItem.name})`; break;
                  case 'egg': gameState.hasEgg = true; gameState.partner = { name: 'Mysterious Egg', isPartner: true, isHatched: false, hatchTime: Date.now() + 7 * 24 * 60 * 60 * 1000, }; rewardText = ' (You found a Mysterious Egg!)'; break;
              }
          }
    ...
    function updatePerksUI() {
          ...
          if (confirmAscensionBtn) { confirmAscensionBtn.textContent = `Ascend to World Tier ${gameState.ascension.tier + 1} (1 AP)`; confirmAscensionBtn.disabled = gameState.level < ASCENSION_LEVEL; }
          }
      }
    function buyPerk(perkId) { const perkData = perks[perkId]; const currentLevel = gameState.ascension.perks[perkId] || 0; if (currentLevel >= perkData.maxLevel) return; const cost = perkData.cost[currentLevel]; if (gameState.ascension.points >= cost) { gameState.ascension.points -= cost; gameState.ascension.perks[perkId] = (gameState.ascension.perks[perkId] || 0) + 1; playSound('levelUp', 0.8, 'sine', 600, 1200, 0.2); updatePerksUI(); updateUI(); saveGame(); } }
    ...
    function buyShopItem(itemId, type) {
          if (type === 'permanent') {
              ...
              if (gameState.gold >= cost) { gameState.gold -= cost; gameState.permanentUpgrades[itemId] = currentLevel + 1; showToast(`Purchased ${upgrade.name} Level ${currentLevel + 1}!`); }
          } else {
    ...
    function updateBuffs() { if (!gameState.activeBuffs) return; let buffsUpdated = false; for (const buffId in gameState.activeBuffs) { if (Date.now() > gameState.activeBuffs[buffId].expiry) { delete gameState.activeBuffs[buffId]; showToast(`${shopItems[buffId].name} has worn off.`); buffsUpdated = true; } } if (buffsUpdated) { updateUI(); saveGame(); } }
    function updateBuffDisplay() { let buffText = ""; if (gameState.activeBuffs) { for (const buffId in gameState.activeBuffs) { const timeLeft = Math.round((gameState.activeBuffs[buffId].expiry - Date.now()) / 1000); buffText += `${shopItems[buffId].name} (${timeLeft}s) `; } } buffDisplay.textContent = buffText; }
    function createLightningEffect() { const bolt = document.createElement('div'); bolt.className = 'lightning-bolt'; bolt.style.left = `${Math.random() * 100}%`; bolt.style.transform = `rotate(${(Math.random() - 0.5) * 20}deg)`; characterArea.appendChild(bolt); setTimeout(() => bolt.remove(), 200); }
    function updateAscensionVisuals() {
          ...
          let newAnimation = 'idle-breathe 4s ease-in-out infinite';
          if (tier >= 5) { characterSprite.classList.add('aura-ascended'); gameScreen.classList.add('background-glitch'); lightningInterval = setInterval(createLightningEffect, 3000); newAnimation = 'idle-breathe 2s ease-in-out infinite, super-aura-anim 1s ease-in-out infinite alternate'; }
          else if (tier >= 3) { characterSprite.classList.add('aura-ascended'); newAnimation = 'idle-breathe 2s ease-in-out infinite, super-aura-anim 1s ease-in-out infinite alternate'; }
          else if (gameState.level >= 10) { characterSprite.classList.add('aura-level-10'); newAnimation = 'idle-breathe 4s ease-in-out infinite, aura-level-10-anim 2s ease-in-out infinite'; }
    ...
    function updateForgeUI() {
          ...
          slots.forEach((slot, i) => { const item = forgeSlots[i]; if (item) { slot.innerHTML = `<strong style="color:${item.rarity.color}">${item.name}</strong><br><small>Reforges: ${item.reforgeCount}/3</small>`; slot.classList.add('filled'); } else { slot.innerHTML = `Slot ${i + 1}`; slot.classList.remove('filled'); } });
          forgeResultSlotDiv.innerHTML = "Result"; forgeResultSlotDiv.classList.remove('filled');
          if (item1 && item2) { const cost = (item1.power + item2.power) * 5; forgeCostDisplay.innerHTML = `Cost: ${cost} Gold, 50 Energy`; }
          else { forgeCostDisplay.innerHTML = ''; }
      }
    function autoForge() {
        if (!confirm("This will automatically forge your weakest items to create stronger ones. This cannot be undone. Are you sure?")) { return; }
        let itemsForged = 0; let keepForging = true;
        while (keepForging) {
            let forgeOccurredThisLoop = false;
            const forgeableWeapons = gameState.inventory.filter(i => i && i.type === 'weapon' && (i.reforgeCount || 0) < 3 && (!gameState.equipment.weapon || i.name !== gameState.equipment.weapon.name)).sort((a, b) => a.power - b.power);
            const forgeableArmor = gameState.inventory.filter(i => i && i.type === 'armor' && (i.reforgeCount || 0) < 3 && (!gameState.equipment.armor || i.name !== gameState.equipment.armor.name)).sort((a, b) => a.power - b.power);
            if (forgeableWeapons.length >= 2) { forgeSlots = [forgeableWeapons[0], forgeableWeapons[1]]; if (forgeItems()) { itemsForged++; forgeOccurredThisLoop = true; continue; } }
            if (forgeableArmor.length >= 2) { forgeSlots = [forgeableArmor[0], forgeableArmor[1]]; if (forgeItems()) { itemsForged++; forgeOccurredThisLoop = true; continue; } }
            keepForging = forgeOccurredThisLoop;
        }
        if (itemsForged > 0) { showToast(`Auto-Forge complete! ${itemsForged} items were created.`); } else { showToast("No forgeable pairs found or not enough resources."); }
        updateInventoryUI(); updateForgeUI();
    }
    function forgeItems() {
        const [item1, item2] = forgeSlots;
        if (!item1 || !item2) { showToast("Need two items to forge."); return false; }
        if (item1.type !== item2.type) { showToast("Items must be the same type."); return false; }
        if ((item1.reforgeCount || 0) >= 3 || (item2.reforgeCount || 0) >= 3) { showToast("One of the items cannot be reforged further."); return false; }
        const cost = Math.floor((item1.power + item2.power) * 5);
        if (gameState.gold < cost) { showToast(`Not enough gold. Need ${formatNumber(cost)} G.`); return false; }
        if (gameState.resources.energy < 50) { showToast("Not enough energy. Need 50."); return false; }
        gameState.gold -= cost; gameState.resources.energy -= 50;
        const newStats = {};
        const allKeys = new Set([...Object.keys(item1.stats), ...Object.keys(item2.stats)]);
        allKeys.forEach(stat => { const val1 = item1.stats[stat] || 0; const val2 = item2.stats[stat] || 0; newStats[stat] = Math.ceil((val1 + val2) * 1.1); });
        ...
        const newItem = { type: item1.type, name: newName, rarity: { key: 'epic', color: newColor }, stats: newStats, power: newPower, reforgeCount: newReforgeCount, refineLevel: 0 };
        ...
        if (gameState.equipment[item1.type] && (gameState.equipment[item1.type].name === item1.name || gameState.equipment[item1.type].name === item2.name)) { gameState.equipment[item1.type] = null; }
    ...
    function sellItemByName(itemName) {
        const itemIndex = gameState.inventory.findIndex(i => i && i.name === itemName);
        if (itemIndex === -1) { console.error("Item to sell not found:", itemName); return; }
        const itemToSell = gameState.inventory[itemIndex];
        if (gameState.equipment[itemToSell.type] && gameState.equipment[itemToSell.type].name === itemToSell.name) { showToast("Cannot sell an equipped item!"); return; }
    ...
    function checkEggHatch() {
          if (gameState.partner && gameState.partner.hatchTime && Date.now() > gameState.partner.hatchTime) {
              gameState.partner.isHatched = true; gameState.partner.hatchTime = null;
              gameState.partner.name = "Newborn Guardian";
              gameState.partner.level = 1; gameState.partner.xp = 0;
    ...
    function updatePartnerUI() {
          ...
          if (partner.isHatched) {
              partnerStatsArea.style.display = 'block'; eggTimerDisplay.style.display = 'none';
              partnerSprite.src = 'player.PNG'; partnerSprite.classList.add('hatched');
              ...
              partnerStatsArea.style.display = 'none'; eggTimerDisplay.style.display = 'block';
              partnerSprite.src = 'egg.png'; partnerSprite.classList.remove('hatched');
              ...
              } else { eggTimerDisplay.textContent = "Hatching..."; }
          }
      }
    ...
    function isNewDay(lastClaimTimestamp) {
        if (lastClaimTimestamp === 0) return true;
        const now = new Date();
        const lastClaim = new Date(lastClaimTimestamp);
        return now.getFullYear() > lastClaim.getFullYear() || now.getMonth() > lastClaim.getMonth() || now.getDate() > lastClaim.getDate();
    }
    function checkOfflineRewards() {
        const now = Date.now();
        const lastLogin = gameState.lastLogin || now;
        const offlineTimeInSeconds = (now - lastLogin) / 1000;
        if (offlineTimeInSeconds < 300) { return; }
        ...
        let itemsFound = [];
        for (let i = 0; i < minutesOffline / 10; i++) { if (Math.random() < 0.75) { itemsFound.push(generateItem()); } }
        const potionsFound = Math.floor(minutesOffline / 120);
        ...
        if (potionsFound > 0) { rewardText += `<br><span style="color:var(--health-color);">Found ${potionsFound} Health Potion(s) 🧪</span>`; }
        if (totalEdgeStones > 0) { rewardText += `<br><span style="color:#00FFFF;">Found ${totalEdgeStones.toFixed(4)} EdgeStones ♦️</span>`; }
    ...
    function checkDailyRewards() {
        if (!isNewDay(gameState.lastDailyClaim)) { return; }
        const now = new Date();
        const lastClaim = new Date(gameState.lastDailyClaim);
        const oneDay = 24 * 60 * 60 * 1000;
        if (now.getTime() - lastClaim.getTime() > (2 * oneDay)) { gameState.dailyStreak = 0; }
        ...
        switch (reward.type) {
            case 'gold': gameState.gold += reward.amount; rewardText = `You received ${reward.amount} Gold!`; break;
            case 'consumable': if (reward.id === 'storableHealthPotion') { gameState.healthPotions = (gameState.healthPotions || 0) + reward.amount; } rewardText = `You received ${reward.amount}x ${shopItems[reward.id].name}!`; break;
            case 'item': const newItem = generateItem(reward.rarity); gameState.inventory.push(newItem); rewardText = `You received a special item: <strong style="color:${newItem.rarity.color}">${newItem.name}</strong>!`; break;
        }
    ...
    function showRewardsModal() {
        ...
        dailyRewards.forEach(reward => {
            ...
            if (reward.day < gameState.dailyStreak) { itemEl.classList.add('claimed'); } else if (reward.day === gameState.dailyStreak && canClaimToday) { itemEl.classList.add('claimable'); }
            let desc = '';
            switch (reward.type) {
                case 'gold': desc = `${reward.amount.toLocaleString()} Gold`; break;
                case 'consumable': desc = `${reward.amount}x ${shopItems[reward.id].name}`; break;
                case 'item': desc = `1x ${reward.rarity.charAt(0).toUpperCase() + reward.rarity.slice(1)} Item`; break;
            }
    ...
    function calculateSellValue(item) {
        if (!item) return 0;
        const rarityMultiplier = { common: 1, uncommon: 2.5, rare: 5, epic: 10, legendary: 25 };
    ...
    function exitDojo() { if (dojoState.isActive) { stopDojoSession(); } showScreen('game-screen'); startGameGenesis(); }
    function updateDojoUI() { dojoPersonalBestDisplay.textContent = `Personal Best: ${formatNumber(Math.floor(gameState.dojoPersonalBest))}`; }
    function startDojoSession() {
        if (dojoState.isActive) return;
        dojoState = { isActive: true, timerId: null, damageIntervalId: null, beamAnimationId: null, totalSessionDamage: 0, timeLeft: 7.0 };
        ...
        dojoState.timerId = setInterval(() => {
            ...
            if (dojoState.timeLeft <= 0) { stopDojoSession(); }
        }, 100);
        dojoState.damageIntervalId = setInterval(() => {
            ...
            const damage = Math.floor(baseDamage * (isCrit ? 2.5 : 1) * DOJO_DAMAGE_MULTIPLIER);
    ...
    async function stopDojoSession() {
        ...
        if (dojoState.totalSessionDamage > gameState.dojoPersonalBest) {
            ...
            updateDojoUI();
            try { await db.collection("damageLeaderboard").doc(gameState.playerName).set({ name: gameState.playerName, totalDamage: Math.floor(gameState.dojoPersonalBest) }, { merge: true }); showToast("New score submitted to leaderboard!"); } catch(e) { console.error("Failed to submit damage score", e); }
            saveGame();
        }
    }
    function applyDamageToEnemy(enemy, damageAmount, isCrit = false) {
        ...
        textEl.style.left = `${enemy.x}px`; textEl.style.top = `${enemy.y - 20}px`; textEl.style.color = isCrit ? 'var(--xp-color)' : '#ff4136';
        if (isCrit) { textEl.style.fontSize = '1.8em'; textEl.style.fontWeight = '900'; } else { textEl.style.fontSize = '1.2em'; }
    ...
    function createDojoDamageNumber(amount, isCrit) {
        ...
        setTimeout(() => { num.style.transform = `translateY(-${80 + Math.random() * 40}px)`; num.style.opacity = '0'; }, 10);
        setTimeout(() => { num.remove(); }, 800);
    }
    function startDojoBeam() {
        if (dojoState.beamAnimationId) cancelAnimationFrame(dojoState.beamAnimationId);
        function drawBeam() {
            const canvas = dojoLightningCanvas; const ctx = dojoCanvasCtx;
    ...
    function stopDojoBeam() { if (dojoState.beamAnimationId) { cancelAnimationFrame(dojoState.beamAnimationId); dojoState.beamAnimationId = null; } dojoCanvasCtx.clearRect(0, 0, dojoLightningCanvas.width, dojoLightningCanvas.height); }
    function drawLightningSegment(ctx, x1, y1, x2, y2, color, lineWidth, jaggedness) {
        const segments = 20; const dx = (x2 - x1) / segments; const dy = (y2 - y1) / segments;
        ctx.beginPath(); ctx.moveTo(x1, y1);
        for (let i = 1; i < segments; i++) {
            const midX = x1 + dx * i; const midY = y1 + dy * i;
            const offsetX = (Math.random() - 0.5) * jaggedness * (1 - (i/segments));
            const offsetY = (Math.random() - 0.5) * jaggedness * (1 - (i/segments));
            ctx.lineTo(midX + offsetX, midY + offsetY);
        }
        ctx.lineTo(x2, y2);
        ctx.strokeStyle = color; ctx.lineWidth = lineWidth; ctx.shadowColor = 'cyan'; ctx.shadowBlur = 20; ctx.stroke();
        ctx.shadowBlur = 0; 
    }
    ...
    function startGameGenesis() {
        ...
        genesisState.player = {
            ...
            x: 0, y: 0,
            isInitialized: false,
            speed: 5, width: 80, height: 80,
            attackRange: 70, lastAttackTime: 0,
            target: null, manualDestination: null,
            isDashing: false, dashTarget: null, dashDuration: 280,
            // Player's manual cooldowns
            lastAttackTime: 0,
            lastDashTime: 0,
            ...
            // --- NEW: AI-specific cooldowns ---
            ai_lastAttackTime: 0, 
            ai_lastDashTime: 0,
            ai_lastThunderStrikeTime: 0,
            ai_lastHavocRageTime: 0,
            // --- END NEW ---
            lastDamagedTime: 0,
            isChargingHavoc: false,
        };
        
        // Add listeners now that the buttons are guaranteed to exist
        havocRageBtn.addEventListener('click', () => manualAbility_HavocRage());
        thunderStrikeBtn.addEventListener('click', () => manualAbility_ThunderStrike());
        dashBtn.addEventListener('click', () => manualAbility_Dash());
    ...
    function stopGameGenesis() {
        ...
        if (genesisState.gameLoopId) { cancelAnimationFrame(genesisState.gameLoopId); genesisState.gameLoopId = null; }
        showMainMenuUI();
    ...
    function toggleGrowthMode() {
        if (genesisState.isActive) {
            stopGameGenesis();
            characterArea.style.display = 'flex'; genesisArena.style.display = 'none';
            growBtn.textContent = 'Endless';
        } else {
            genesisArena.style.display = 'block'; characterArea.style.display = 'none';
            startGameGenesis();
    ...
    function handleLootCollection() {
        ...
        genesisState.lootOrbs = genesisState.lootOrbs.filter(orb => {
            ...
            if (distance <= orb.collectionRadius) {
                ...
                const screenX = arenaRect.left + player.x; const screenY = arenaRect.top + player.y;
    ...
    function toggleAutoBattle() {
        ...
        gameState.settings.isAutoBattle = !gameState.settings.isAutoBattle;
        updateAutoBattleUI();
        saveGame();

        // --- THIS IS THE CORRECTED "WAKE UP" LOGIC ---
        if (gameState.settings.isAutoBattle) {
            // We just turned Auto-Battle ON.
            // Clear any manual clicks and force the AI to find a new target immediately.
            genesisState.player.manualDestination = null;
            updatePlayerTarget();
        }
    }
    function updateAutoBattleUI() {
        // Sync the main menu checkbox
        if(autoBattleToggle) {
            autoBattleToggle.checked = gameState.settings.isAutoBattle;
        }
    
        // Sync the in-game round button's appearance
        if(inGameAutoBtn) {
            if (gameState.settings.isAutoBattle) {
                inGameAutoBtn.classList.add('active');
                inGameAutoBtn.textContent = 'Auto';
            } else {
                inGameAutoBtn.classList.remove('active');
                inGameAutoBtn.textContent = 'Manual';
            }
        }
    
        // Hide/Show ability buttons based on the state
        if(havocRageBtn) havocRageBtn.style.display = gameState.settings.isAutoBattle ? 'none' : 'flex';
        if(thunderStrikeBtn) thunderStrikeBtn.style.display = gameState.settings.isAutoBattle ? 'none' : 'flex';
        if(dashBtn) dashBtn.style.display = gameState.settings.isAutoBattle ? 'none' : 'flex';
    }

    // =======================================================
    // --- PASTE THE NEW "AI BRAIN" FUNCTION HERE ---
    // =======================================================
    function makeAIDecision(player, timestamp) {
      // AI Decision Priority:
      // 1. Use big cooldowns if available.
      // 2. Reposition with Dash if enemies are clustered.
      // 3. Use basic attacks if in range.
      // 4. Move towards the target if nothing else to do.

      // --- Decision 1: Use Havoc Rage if ready ---
      if (timestamp - player.ai_lastHavocRageTime > player.havocRageCooldown && genesisState.enemies.length > 2) { // <-- Use ai_
        return 'havocRage';
    }

      // --- Decision 2: Use Thunder Strike if ready and an enemy is close ---
      const thunderStrikeProximity = 50;
      let isEnemyCloseForThunder = false;
      for (const enemy of genesisState.enemies) {
          const dx = player.x - enemy.x;
          const dy = player.y - enemy.y;
          if (Math.sqrt(dx * dx + dy * dy) <= thunderStrikeProximity) {
              isEnemyCloseForThunder = true;
              break;
          }
      }
      if (timestamp - player.ai_lastThunderStrikeTime > player.thunderStrikeCooldown && isEnemyCloseForThunder) { // <-- Use ai_
        return 'thunderStrike';
    }
      
      // --- Decision 3: Use Dash to engage a large cluster ---
      if (timestamp - player.ai_lastDashTime > player.dashCooldown) {
          let bestTarget = null;
          let maxCount = 0;
          const clusterRadius = 150;
          // Find the best cluster to dash into
          ...
          // Only dash if there are at least 3 enemies in the target cluster
          if (bestTarget && maxCount >= 3) {
              return 'dash';
          }
      }

      // --- Decision 4: Use Basic Attack if in range ---
      if (player.target) {
        ...
        // --- THIS IS THE FIX ---
        // The AI now checks its OWN timer (ai_lastAttackTime)
        if (distanceToTarget <= player.attackRange && (timestamp - player.ai_lastAttackTime > dynamicAttackCooldown)) {
            return 'attack';
        }
    }

      // --- Default Decision: Move towards target ---
      return 'move';
  }

    function gameLoop(timestamp) {
        if (!genesisState.isActive || !genesisState.player) return;
        if (timestamp - genesisState.player.lastDamagedTime > 200) { genesisState.player.element.style.filter = ''; }
        if (!genesisState.isBattleMode) { const DIFFICULTY_INTERVAL = 15000; if (timestamp - genesisState.lastDifficultyIncrease > DIFFICULTY_INTERVAL) { if (genesisState.lastDifficultyIncrease !== 0) { genesisState.difficultyLevel++; showToast(`Challenge Level ${genesisState.difficultyLevel}!`); playSound('levelUp', 0.5, 'sawtooth', 300, 400, 0.2); } genesisState.lastDifficultyIncrease = timestamp; } }
        if (genesisState.isBattleMode && genesisState.enemies.length === 0 && genesisState.enemiesSpawnedThisWave >= genesisState.enemiesToSpawnThisWave && !genesisState.waveTransitionActive && genesisState.currentWave < genesisState.totalWaves) {
            ...
            setTimeout(() => { if (!genesisState.isActive || !genesisState.waveTransitionActive) return; if (genesisState.currentWave + 1 === genesisState.totalWaves) { genesisWaveDisplay.textContent = 'BOSS INCOMING!'; playSound('ascend', 0.7, 'sawtooth', 500, 100, 0.4); } else { genesisWaveDisplay.textContent = 'Next Wave Incoming...'; } }, 1500);
            setTimeout(() => { if (!genesisState.isActive) return; startNextBattleWave(); if (genesisState.isActive) { genesisState.waveTransitionActive = false; } }, 3000);
        }
        updatePlayerTarget();
        movePlayer();
        moveEnemies();
        ...
        if (gameState.settings.isAutoBattle) {
            const decision = makeAIDecision(genesisState.player, timestamp);

            switch (decision) {
                case 'havocRage':
                    handleGenesisHavocRage(timestamp);
                    break;
                case 'thunderStrike':
                    handleGenesisThunderStrike(timestamp);
                    break;
                case 'dash':
                    initiateGenesisPlayerDash(timestamp);
                    break;
                case 'attack':
                    handleGenesisPlayerAttack(timestamp);
                    break;
                // 'move' is the default and is handled by movePlayer(), so we do nothing.
            }
        }
        ...
        genesisState.enemies.forEach(enemy => { enemy.element.style.left = `${enemy.x}px`; enemy.element.style.top = `${enemy.y}px`; });
        genesisState.lootOrbs.forEach(orb => { const orbTranslateX = orb.x - (orb.element.offsetWidth / 2); const orbTranslateY = orb.y - (orb.element.offsetHeight / 2); orb.element.style.transform = `translate(${orbTranslateX}px, ${orbTranslateY}px)`; });
        spawnEnemies(timestamp);
        if (genesisState.isBattleMode && genesisState.boss && genesisState.boss.hp <= 0 && genesisState.isActive) { genesisState.isActive = false; endBattle(true); return; }
        if (gameState.resources.hp <= 0 && genesisState.isActive) { genesisState.isActive = false; endBattle(false); return; }
        genesisState.gameLoopId = requestAnimationFrame(gameLoop);
    }
    function spawnEnemies(timestamp) {
        if (genesisState.isBattleMode) {
            ...
            if (genesisState.enemiesSpawnedThisWave < genesisState.enemiesToSpawnThisWave && timestamp - genesisState.lastEnemySpawn > waveSpawnInterval) {
                ...
                const enemy = { element: document.createElement('img'), healthBarContainer: document.createElement('div'), healthBarFill: document.createElement('div'), x: 0, y: 0, speed: (0.5 + Math.random() * 2.0) * waveMultiplier, width: 40, height: 40, maxHp: (40 + (10 * difficulty)) * waveMultiplier, hp: (40 + (10 * difficulty)) * waveMultiplier, id: Date.now() + Math.random(), attackRange: 25, attackCooldown: 2000, lastAttackTime: 0 };
                const padding = 50;
            ...
            if (timestamp - genesisState.lastEnemySpawn > dynamicSpawnInterval && arenaRect.width > 0) {
                ...
                const enemy = { element: document.createElement('img'), healthBarContainer: document.createElement('div'), healthBarFill: document.createElement('div'), x: 0, y: 0, speed: enemySpeed, width: 40, height: 40, maxHp: enemyHp, hp: enemyHp, id: Date.now() + Math.random(), attackRange: 25, attackCooldown: 2000, lastAttackTime: 0, };
                const padding = 50;
                enemy.x = padding + (Math.random() * (arenaRect.width - padding * 2));
                enemy.y = padding + (Math.random() * (arenaRect.height - padding * 2));
                enemy.healthBarContainer.className = 'enemy-health-bar-container';
    ...
    function movePlayer() {
        ...
        if (player.isDashing && player.dashTarget) { targetX = player.dashTarget.x; targetY = player.dashTarget.y; currentSpeed = player.speed * 5; createDashTrailEffect(player); } 
        else if (player.manualDestination) {
            targetX = player.manualDestination.x;
            targetY = player.manualDestination.y;
            const dx = targetX - player.x;
            const dy = targetY - player.y;
            if (Math.sqrt(dx * dx + dy * dy) < 10) { player.manualDestination = null; }
        } 
        else {
            if (player.target) {
                if (genesisState.isBattleMode) {
                    const enemyTargetX = player.target.x; const enemyTargetY = player.target.y;
                    const vectorX = enemyTargetX - player.patrolCenterX; const vectorY = enemyTargetY - player.patrolCenterY;
                    const distFromPatrolCenter = Math.sqrt(vectorX * vectorX + vectorY * vectorY);
                    if (distFromPatrolCenter > player.patrolRadius) { const normX = vectorX / distFromPatrolCenter; const normY = vectorY / distFromPatrolCenter; targetX = player.patrolCenterX + normX * player.patrolRadius; targetY = player.patrolCenterY + normY * player.patrolRadius; }
                    else { targetX = enemyTargetX; targetY = enemyTargetY; }
                }
                else { const dx = player.target.x - player.x; const dy = player.target.y - player.y; const distanceToTarget = Math.sqrt(dx*dx + dy*dy); if (distanceToTarget > player.attackRange * 0.4) { targetX = player.target.x; targetY = player.target.y; } }
            }
            else { if (genesisState.isBattleMode) { const dx = player.patrolCenterX - player.x; const dy = player.patrolCenterY - player.y; if (Math.sqrt(dx * dx + dy * dy) > 5) { targetX = player.patrolCenterX; targetY = player.patrolCenterY; } } }
        }
        if (targetX !== null && targetY !== null) {
            const moveDx = targetX - player.x; const moveDy = targetY - player.y;
            const moveDistance = Math.sqrt(moveDx * moveDx + moveDy * moveDy);
            if (moveDistance > 1) { const moveAmount = Math.min(currentSpeed, moveDistance); player.x += (moveDx / moveDistance) * moveAmount; player.y += (moveDy / moveDistance) * moveAmount; }
        }
        const arenaRect = genesisArena.getBoundingClientRect();
        if (arenaRect.width > 0) { player.x = clamp(player.x, player.width / 2, arenaRect.width - player.width / 2); player.y = clamp(player.y, player.height / 2, arenaRect.height - player.height / 2); }
    }
    function initiateGenesisPlayerDash(timestamp) {
        ...
        genesisState.enemies.forEach(potentialTarget => {
            let nearbyCount = 0;
            genesisState.enemies.forEach(otherEnemy => { const dx = potentialTarget.x - otherEnemy.x; const dy = potentialTarget.y - otherEnemy.y; if (Math.sqrt(dx * dx + dy * dy) < clusterRadius) { nearbyCount++; } });
            if (nearbyCount > maxCount) { maxCount = nearbyCount; bestTarget = potentialTarget; }
        });

        if (!bestTarget) { bestTarget = player.target; }

        if (bestTarget) {
            const dx = player.x - bestTarget.x; const dy = player.y - bestTarget.y;
            const distanceToTarget = Math.sqrt(dx * dx + dy * dy);
            const maxDashRange = 600; 
            if (distanceToTarget > maxDashRange) { return false; }

            // --- THIS IS THE FIX ---
            if (gameState.settings.isAutoBattle) {
                player.ai_lastDashTime = timestamp;
            } else {
            ...
            const arenaRect = genesisArena.getBoundingClientRect();
            const screenX = arenaRect.left + player.x; const screenY = arenaRect.top + player.y - 80;
            if (shouldChain) { createFloatingText("Dash Chain!", screenX, screenY, { color: '#ff8c00', fontSize: '2.2em' }); } else { createFloatingText("Dash!", screenX, screenY, { color: '#ff8c00', fontSize: '1.8em' }); }
    ...
    function performDash(target, currentChain, maxChains) {
        const player = genesisState.player;
        if (!player || !target) { if(player) player.isDashing = false; return; };
        player.isDashing = true;
        player.dashTarget = { x: target.x, y: target.y };
        playSound('hit', 1, 'sawtooth', 800, 200, 0.2);
        setTimeout(() => {
            if (!genesisState.player) return;
            const explosionX = player.x; const explosionY = player.y;
            ...
            genesisState.enemies.forEach(enemy => {
                const dx = explosionX - enemy.x; const dy = explosionY - enemy.y;
                if (Math.sqrt(dx * dx + dy * dy) <= explosionRadius) { const isCrit = Math.random() < (getTotalStat('critChance') / 100); const finalDamage = dashDamage * (isCrit ? 2.5 : 1); applyDamageToEnemy(enemy, finalDamage, isCrit); }
            });
            genesisState.enemies = genesisState.enemies.filter(e => e.hp > 0);
            if (currentChain < maxChains && genesisState.enemies.length > 0) {
                let furthestEnemy = null;
                let maxDistance = -1;
                genesisState.enemies.forEach(enemy => { const dx = player.x - enemy.x; const dy = player.y - enemy.y; const distance = Math.sqrt(dx * dx + dy * dy); if (distance > maxDistance) { maxDistance = distance; furthestEnemy = enemy; } });
                if (furthestEnemy) { performDash(furthestEnemy, currentChain + 1, maxChains); } else { player.isDashing = false; }
    ...
    function createHavocShockwaveEffect(x, y, range) {
        ...
        for (let i = 0; i < numberOfBolts; i++) {
            const bolt = document.createElement('div'); bolt.className = 'havoc-starburst-bolt';
            bolt.style.left = `${x}px`; bolt.style.top = `${y}px`;
            const angle = (360 / numberOfBolts) * i;
            bolt.style.transform = `rotate(${angle}deg)`;
            bolt.style.animationDelay = `${Math.random() * 0.15}s`;
            genesisArena.appendChild(bolt);
            setTimeout(() => { if (bolt) bolt.remove(); }, 600);
        }
    }
    function createBurnParticles(enemy) {
        if (!enemy || !enemy.element) return;
        const particle = document.createElement('div'); particle.className = 'enemy-fire-particle';
        const offsetX = (Math.random() - 0.5) * enemy.width; const offsetY = (Math.random() - 0.5) * enemy.height;
        particle.style.left = `${enemy.x + offsetX}px`; particle.style.top = `${enemy.y + offsetY}px`;
        genesisArena.appendChild(particle);
        setTimeout(() => particle.remove(), 500);
    }
    function createChainLightningEffect(targets) {
        const canvas = document.createElement('canvas'); canvas.className = 'genesis-chain-lightning';
        const arenaRect = genesisArena.getBoundingClientRect();
        canvas.width = arenaRect.width; canvas.height = arenaRect.height;
        genesisArena.appendChild(canvas);
        const ctx = canvas.getContext('2d');
        for (let i = 0; i < targets.length - 1; i++) {
            const start = targets[i]; const end = targets[i+1];
    ...
    function handleGenesisThunderStrike(timestamp) {
        ...
        // The proximity check is now done in the AI brain, but must remain here for the manual click to work.
        const thunderStrikeProximity = 50;
        let isEnemyClose = false;
        for (const enemy of genesisState.enemies) { const dx = player.x - enemy.x; const dy = player.y - enemy.y; if (Math.sqrt(dx * dx + dy * dy) <= thunderStrikeProximity) { isEnemyClose = true; break; } }
        if (!isEnemyClose) { return false; }
        ...
        for (let i = 0; i < maxChain && potentialTargets.length > 0; i++) {
            ...
            potentialTargets.forEach(enemy => { const dx = lastTarget.x - enemy.x; const dy = lastTarget.y - enemy.y; const distance = Math.sqrt(dx * dx + dy * dy); if (distance < minDistance) { minDistance = distance; closestEnemy = enemy; } });
            if (closestEnemy) { chainTargets.push(closestEnemy); potentialTargets = potentialTargets.filter(e => e.id !== closestEnemy.id); }
        }

        if (chainTargets.length > 1) {
            // --- THIS IS THE FIX ---
            if (gameState.settings.isAutoBattle) {
                player.ai_lastThunderStrikeTime = timestamp;
            } else {
            ...
            const arenaRect = genesisArena.getBoundingClientRect();
            const screenX = arenaRect.left + player.x; const screenY = arenaRect.top + player.y - 80;
            createFloatingText("Thunder Strike!", screenX, screenY, { color: '#00ffff', fontSize: '2.2em' });
            createChainLightningEffect(chainTargets); 
            const thunderDamage = getTotalStat('strength') * 4.5;
            for (let i = 1; i < chainTargets.length; i++) { const enemy = chainTargets[i]; const isCrit = Math.random() < (getTotalStat('critChance') / 100); const finalDamage = thunderDamage * (isCrit ? 2.5 : 1); applyDamageToEnemy(enemy, finalDamage, isCrit); }
            genesisState.enemies = genesisState.enemies.filter(e => e.hp > 0);
            return true;
        }
        return false;
    ...
    function handleGenesisHavocRage(timestamp) {
        ...
        // --- THIS IS THE FIX ---
        if (gameState.settings.isAutoBattle) {
            player.ai_lastHavocRageTime = timestamp;
        } else {
        ...
        const arenaRect = genesisArena.getBoundingClientRect();
        const screenX = arenaRect.left + player.x; const screenY = arenaRect.top + player.y - 80;
        createFloatingText("Havoc Rage!", screenX, screenY, { color: '#dc143c', fontSize: '2.2em' });
        player.element.style.filter = 'invert(1)';
        setTimeout(() => { if (player && player.element) { player.element.style.filter = ''; } }, 250);
        const effectRadius = 200;
        createHavocShockwaveEffect(player.x, player.y, effectRadius);
        genesisState.enemies.forEach(enemy => { enemy.burn = { expiryTime: timestamp + 5000, damagePerTick: getTotalStat('strength') * 0.25, lastTickTime: timestamp }; enemy.element.classList.add('burning'); });
        return true;
    }
    function handleBurnDamage(timestamp) {
        ...
        for (let i = genesisState.enemies.length - 1; i >= 0; i--) {
            ...
            if (enemy && enemy.burn) {
                if (timestamp > enemy.burn.expiryTime) { delete enemy.burn; if(enemy.element) enemy.element.classList.remove('burning'); continue; }
                if (timestamp - enemy.burn.lastTickTime > 250) {
                    ...
                    if (enemy.hp > 0) { enemy.burn.lastTickTime = timestamp; }
                }
            }
        }
        genesisState.enemies = genesisState.enemies.filter(e => e.hp > 0);
    ...
    function handleGenesisPlayerAttack(timestamp) {
        const player = genesisState.player;
        // The checks for cooldown, target, and range are now removed from here,
        // as they are handled by makeAIDecision (for the AI) and the click handler (for the player).

        // --- THIS IS THE CORE FIX ---
        // We now set the correct timer based on who is attacking.
        if (gameState.settings.isAutoBattle) {
            player.ai_lastAttackTime = timestamp;
        } else {
    ...
    function createSlashEffect(source, target) {
        const slashEl = document.createElement('div'); slashEl.className = 'genesis-slash';
        genesisArena.appendChild(slashEl);
        const midX = (source.x + target.x) / 2; const midY = (source.y + target.y) / 2;
        const angle = Math.atan2(target.y - source.y, target.x - source.x) * (180 / Math.PI);
        slashEl.style.left = `${midX}px`; slashEl.style.top = `${midY}px`;
    ...
    function createAoeSlashEffect(x, y, range) {
        const pulseEl = document.createElement('div'); pulseEl.className = 'genesis-aoe-pulse';
        pulseEl.style.width = `${range * 2}px`; pulseEl.style.height = `${range * 2}px`;
        pulseEl.style.left = `${x}px`; pulseEl.style.top = `${y}px`;
        genesisArena.appendChild(pulseEl);
        setTimeout(() => pulseEl.remove(), 300);
    }
    function createImpactEffect(x, y) {
        const impactEl = document.createElement('div'); impactEl.className = 'genesis-impact';
        genesisArena.appendChild(impactEl);
        impactEl.style.left = `${x}px`; impactEl.style.top = `${y}px`;
    ...
    function createDashTrailEffect(player) {
        ...
        trailEl.style.left = `${player.x}px`; trailEl.style.top = `${player.y}px`;
        genesisArena.appendChild(trailEl);
        requestAnimationFrame(() => { trailEl.style.opacity = '0'; });
    ...
    function createLootOrb(x, y) {
        if (genesisState.isBattleMode) { return; }
        const orb = { element: document.createElement('div'), x: x, y: y, type: 'gold', amount: Math.floor(5 + (Math.random() * 5 * gameState.level)), collectionRadius: 690, magnetRadius: 2550, magnetSpeed: 13 };
    ...
    function createPvpDamageNumber(amount, isPlayerSource) {
        const numEl = document.createElement('div'); numEl.className = 'enemy-damage-text';
        numEl.textContent = formatNumber(Math.floor(amount));
        if (isPlayerSource) { numEl.style.left = '75%'; numEl.style.color = 'var(--accent-color)'; } else { numEl.style.left = '25%'; numEl.style.color = 'var(--health-color)'; }
    ...
    function moveLootOrbs() {
        ...
        genesisState.lootOrbs.forEach(orb => {
            const dx = player.x - orb.x; const dy = player.y - orb.y;
            const distance = Math.sqrt(dx * dx + dy * dy);
            if (distance < orb.magnetRadius && distance > 1) { orb.x += (dx / distance) * orb.magnetSpeed; orb.y += (dy / distance) * orb.magnetSpeed; }
        });
    }
    ...
    function startNextBattleWave() {
        ...
        if (genesisState.currentWave > genesisState.totalWaves) { endBattle(true); return; }
        ...
        if (genesisState.currentWave === genesisState.totalWaves) {
            ...
            const baseMinEnemies = 3; const baseMaxEnemies = 18;
            const waveBonus = Math.floor(genesisState.currentWave * 1.5);
            const minEnemies = baseMinEnemies + waveBonus; const maxEnemies = baseMaxEnemies + waveBonus;
    ...
    function spawnBoss() {
        const arenaRect = genesisArena.getBoundingClientRect();
        const difficulty = gameState.highestBattleLevelCompleted + 1;
        const boss = { element: document.createElement('img'), x: arenaRect.width - 100, y: arenaRect.height / 2, speed: 1, width: 160, height: 160, maxHp: 500 * difficulty * gameState.ascension.tier, hp: 500 * difficulty * gameState.ascension.tier, id: 'BOSS', isBoss: true, attackRange: 80, attackCooldown: 1000, lastAttackTime: 0 };
        boss.element.src = 'player.PNG';
        boss.element.className = 'genesis-enemy';
        boss.element.style.width = `${boss.width}px`; boss.element.style.height = `${boss.height}px`;
    ...
    function updateBossHealthBar() { if (genesisState.boss) { const percent = (genesisState.boss.hp / genesisState.boss.maxHp) * 100; bossHealthFill.style.width = `${percent}%`; } }
    async function endBattle(playerWon) {
        if (genesisState.isBattleMode && genesisState.totalDamageDealtThisBattle > gameState.dojoPersonalBest) {
            ...
            playSound('victory', 1, 'triangle', 523, 1046, 0.4);
            updateDojoUI();
            try { await db.collection("damageLeaderboard").doc(gameState.playerName).set({ name: gameState.playerName, totalDamage: Math.floor(gameState.dojoPersonalBest) }, { merge: true }); showToast("New damage score submitted to leaderboard!"); } catch(e) { console.error("Failed to submit damage score", e); }
        }
        ...
        if (playerWon) {
            ...
            if (gameState.resources.hp <= 0) { title = "Defeated!"; rewardText = "You black out and wake up back home. You lost half your current gold."; gameState.gold = Math.floor(gameState.gold / 2); gameState.resources.hp = 1; }
            else { title = "Fled from Battle"; rewardText = "You escaped, but gained no rewards."; }
        }
        genesisState.isActive = false;
        if (genesisState.gameLoopId) { cancelAnimationFrame(genesisState.gameLoopId); genesisState.gameLoopId = null; }
        const dynamicElements = genesisArena.querySelectorAll('.genesis-player, .genesis-enemy, .genesis-loot-orb');
        dynamicElements.forEach(el => el.remove());
        genesisState.player = null; genesisState.enemies = [];
        if(genesisState.lootOrbs) genesisState.lootOrbs = [];
        genesisWaveDisplay.style.display = 'none';
        bossHealthContainer.style.display = 'none';
        genesisState.isBattleMode = false;
    ...
    function passiveResourceRegen() {
          ...
          if (gameState.resources && !gameState.expedition.active && !battleState.isActive) {
              if (gameState.resources.hp < gameState.resources.maxHp) { gameState.resources.hp = Math.min(gameState.resources.maxHp, gameState.resources.hp + getTotalStat('stamina') * 0.05); playerUINeedsUpdate = true; }
              if (gameState.resources.energy < gameState.resources.maxEnergy) { gameState.resources.energy = Math.min(gameState.resources.maxEnergy, gameState.resources.energy + 0.15); playerUINeedsUpdate = true; }
          }
          if (gameState.partner && gameState.partner.isHatched) {
              if (gameState.partner.resources.hp < gameState.partner.resources.maxHp) { gameState.partner.resources.hp = Math.min(gameState.partner.resources.maxHp, gameState.partner.resources.hp + gameState.partner.stats.stamina * 0.05); partnerUINeedsUpdate = true; }
              if (gameState.partner.resources.energy < gameState.partner.resources.maxEnergy) { gameState.partner.resources.energy = Math.min(gameState.partner.resources.maxEnergy, gameState.partner.resources.energy + 0.15); partnerUINeedsUpdate = true; }
          }
          if (playerUINeedsUpdate) { updateUI(); }
          if (partnerUINeedsUpdate) { updatePartnerUI(); }
      }
    function signInWithGoogle() { auth.signInWithPopup(googleProvider).then(result => { showToast(`Welcome, ${result.user.displayName}!`); }).catch(error => { console.error("Sign in error", error); showToast(`Sign in failed: ${error.message}`); }); }
    function signOut() {
        ...
        auth.signOut().then(() => {
            ...
            if (localSaveExists) {
                if (confirm("You are now signed out. Do you want to load your local save file? (This will not affect your cloud save)")) {
                    // We must re-enable the button because the user is logged out now
                    loadGameBtn.disabled = false;
                    loadGame();
                } else { showScreen('main-menu-screen'); updateUI(); }
            } else { showScreen('main-menu-screen'); updateUI(); }
        });
      }
    function updateAuthUI(user) {
          if (user) { authStatus.textContent = `Signed in as ${user.displayName || user.email}`; googleSigninBtn.textContent = 'Sign Out'; loadGameBtn.disabled = false; }
          else { authStatus.textContent = 'Sign in for Cloud Saves & Leaderboards'; googleSigninBtn.textContent = 'Sign in with Google'; loadGameBtn.disabled = !localStorage.getItem('tapGuardianSave'); }
      }
    function updateSettingsUI() {
        ...
        autoBattleToggle.checked = gameState.settings.isAutoBattle; 
        if(musicManager.currentTrack) { musicManager.audio[musicManager.currentTrack].volume = gameState.settings.isMuted ? 0 : gameState.settings.musicVolume; }
    }
    async function enterPvpSelection() {
        if (gameState.level < PVP_UNLOCK_LEVEL) { showToast(`PvP unlocks at Level ${PVP_UNLOCK_LEVEL}`); return; }
        showScreen('pvp-selection-screen');
        pvpOpponentListContainer.innerHTML = '<p>Searching for opponents...</p>';
        try {
            const snapshot = await db.collection('playerSaves').where('level', '>=', PVP_UNLOCK_LEVEL).limit(50).get();
            let allEligiblePlayers = [];
            snapshot.forEach(doc => { const data = doc.data(); if (data.playerName !== gameState.playerName) { allEligiblePlayers.push(data); } });
            const lowerBound = gameState.level - 10;
            const upperBound = gameState.level + 10;
            let opponents = allEligiblePlayers.filter(player => { return player.level >= lowerBound && player.level <= upperBound; });
            if (opponents.length < 1) { opponents = allEligiblePlayers; }
            if (opponents.length === 0) { pvpOpponentListContainer.innerHTML = '<p>Could not find any eligible opponents. Please try again later.</p>'; return; }
            ...
            opponents.slice(0, 4).forEach(opponent => {
                ...
                itemEl.innerHTML = `<img src="player.PNG" class="pvp-opponent-avatar" style="filter: hue-rotate(${Math.random() * 360}deg);"><div class="pvp-opponent-info"><div class="pvp-opponent-name">${opponent.playerName} (Lv. ${opponent.level})</div><div class="pvp-opponent-stats">Tier: ${opponent.ascension.tier}</div></div>`;
                itemEl.onclick = () => startPvpBattle(opponent);
                pvpOpponentListContainer.appendChild(itemEl);
            });
        } catch (error) { console.error("Error fetching PvP opponents:", error); if (error.message.includes('index')) { pvpOpponentListContainer.innerHTML = `<p>Database requires an index. Please check the developer console (F12) for a link to create it.</p>`; } else { pvpOpponentListContainer.innerHTML = '<p>Could not find opponents. Please try again later.</p>'; } }
    }
    function startPvpBattle(opponentData) {
           pvpState = { isActive: true, timerId: null, timeLeft: 15.0, playerDamage: 0, opponentDamage: 0, opponentData: opponentData, playerCooldowns: { lastDashTime: 0, lastThunderStrikeTime: 0, lastHavocRageTime: 0, lastAttackTime: 0 }, opponentCooldowns: { lastDashTime: 0, lastThunderStrikeTime: 0, lastHavocRageTime: 0, lastAttackTime: 0 } };
           showScreen('pvp-battle-screen');
           pvpArena.innerHTML = '';
           const playerSprite = document.createElement('img');
           playerSprite.src = 'player.PNG'; playerSprite.className = 'genesis-player'; playerSprite.style.left = '25%'; playerSprite.style.top = '50%';
           const opponentSprite = document.createElement('img');
           opponentSprite.src = 'player.PNG'; opponentSprite.className = 'genesis-player'; opponentSprite.style.filter = 'hue-rotate(180deg)'; opponentSprite.style.left = '75%'; opponentSprite.style.top = '50%';
           pvpArena.appendChild(playerSprite); pvpArena.appendChild(opponentSprite);
    ...
    function executePvpTurn(isPlayer, timestamp) {
        ...
        const getCombatantStat = (stat) => { if (isPlayer) return getTotalStat(stat); let total = combatant.stats[stat] || 0; for (const slot in combatant.equipment) { if (combatant.equipment[slot] && combatant.equipment[slot].stats) { total += combatant.equipment[slot].stats[stat] || 0; } } return total; };
        if (timestamp - cooldowns.lastThunderStrikeTime > 4000) {
            ...
            createPvpDamageNumber(finalDamage, isPlayer); return;
        }
        if (timestamp - cooldowns.lastHavocRageTime > 12000) {
            ...
            createPvpDamageNumber(damage, isPlayer); return;
        }
    ...
    function pvpTick() {
        if (!pvpState.isActive) { clearInterval(pvpState.timerId); return; }
        pvpState.timeLeft -= 0.1;
        if (pvpState.timeLeft <= 0) { pvpTimerDisplay.textContent = "0.0"; endPvpBattle(); return; }
        pvpTimerDisplay.textContent = pvpState.timeLeft.toFixed(1);
        const timestamp = Date.now();
        const sprites = pvpArena.querySelectorAll('.genesis-player');
        if (sprites.length === 2) {
            const playerSprite = sprites[0]; const opponentSprite = sprites[1];
            const distance = opponentSprite.offsetLeft - playerSprite.offsetLeft;
            if (distance > 70) { playerSprite.style.left = `${playerSprite.offsetLeft + 2}px`; opponentSprite.style.left = `${opponentSprite.offsetLeft - 2}px`; }
        }
    ...
    async function endPvpBattle() {
        ...
        if (playerWon) {
            ...
            if (pvpState.playerDamage > pvpBest) {
                ...
                try { await db.collection("pvpLeaderboard").doc(gameState.playerName).set({ name: gameState.playerName, maxDamage: Math.floor(gameState.pvpPersonalBest) }, { merge: true }); }
                catch(e) { console.error("Failed to submit PvP score", e); }
            }
        } else { playSound('defeat', 1, 'sine', 440, 110, 0.8); }
   ...
   const handleCharacterTouchStart = (e) => { e.preventDefault(); handleTap(e.touches[0], false); };
   const handlePartnerTouchStart = (e) => { e.preventDefault(); handleTap(e.touches[0], true); };
   characterSprite.addEventListener('touchstart', handleCharacterTouchStart, {passive: false});
   partnerSprite.addEventListener('touchstart', handlePartnerTouchStart, {passive: false});
   ...
   genesisArena.addEventListener('click', (e) => {
       ...
       if (!gameState.settings.isAutoBattle) {
           ...
           genesisState.enemies.forEach(enemy => { const dx = clickX - enemy.x; const dy = clickY - enemy.y; if (Math.sqrt(dx * dx + dy * dy) < enemy.width / 2) { clickedOnEnemy = true; } });
           if (clickedOnEnemy) {
            // --- THIS IS THE FIX ---
            const player = genesisState.player;
            const agility = getTotalStat('agility');
            const dynamicAttackCooldown = Math.max(95, 300 - (agility * 1));
            // Only attack if the PLAYER'S cooldown is ready.
            if (Date.now() - player.lastAttackTime > dynamicAttackCooldown) {
                handleGenesisPlayerAttack(Date.now());
            }
            // --- END OF FIX ---
        }
    }
   ...
   autoBattleToggle.addEventListener('change', toggleAutoBattle);
   inGameAutoBtn.addEventListener('click', toggleAutoBattle);
   uiToggleBtn.addEventListener('click', () => {
    actionButtons.style.display = (actionButtons.style.display === 'none') ? 'grid' : 'none';
    uiToggleBtn.classList.toggle('active');
   });
   modalCloseBtn.addEventListener('click', () => modal.classList.remove('visible'));
   ingameMenuBtn.addEventListener('click', () => ingameMenuModal.classList.add('visible'));
   returnToGameBtn.addEventListener('click', () => ingameMenuModal.classList.remove('visible'));
   inventoryBtn.addEventListener('click', () => { currentForgeSelectionTarget = null; updateInventoryUI(); inventoryModal.classList.add('visible'); });
   closeInventoryBtn.addEventListener('click', () => { currentForgeSelectionTarget = null; inventoryModal.classList.remove('visible'); });
   ...
   playerStatPanel.addEventListener('click', (event) => { if (event.target && event.target.id === 'rewards-btn') { showRewardsModal(); } });
   ...
   expeditionCancelBtn.addEventListener('click', () => { showScreen('game-screen'); startGameGenesis(); });
   ...
   dojoDummySprite.addEventListener('touchstart', (e) => { e.preventDefault(); startDojoSession(); }, { passive: false });
   ...
   pvpSelectionBackBtn.addEventListener('click', () => { showScreen('game-screen'); startGameGenesis(); });
   ...
   [forgeSlot1Div, forgeSlot2Div].forEach((slot, index) => {
       slot.addEventListener('click', () => {
           if (forgeSlots[index]) { forgeSlots[index] = null; updateForgeUI(); }
           else { openInventoryForForgeSelection(index); }
       });
   });
   leaderboardTabs.forEach(tab => tab.addEventListener('click', () => showLeaderboard(tab.dataset.type)));
   googleSigninBtn.addEventListener('click', () => { if(auth.currentUser) { signOut(); } else { signInWithGoogle(); } });
   muteAllCheckbox.addEventListener('change', (e) => { gameState.settings.isMuted = e.target.checked; updateSettingsUI(); });
   musicVolumeSlider.addEventListener('input', (e) => { gameState.settings.musicVolume = parseFloat(e.target.value); updateSettingsUI(); });
