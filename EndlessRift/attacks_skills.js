...
// MODIFIED: fireProjectile now ensures projectiles are removed based on world bounds, not just camera bounds.
function fireProjectile(p) {
    ...
    // MODIFIED: Corrected diagonal shot to fire all 4 diagonals
    if (p.abilities.diagonalShot) { fire(Math.PI / 4); fire(-Math.PI / 4); fire(Math.PI * 3 / 4); fire(-Math.PI * 3 / 4); }
}
...
function createXpOrb(x, y, value, player, gainXPCallback) {
    ...
    const newOrb = {
        ...
        update(dt, options) { // options contains player and gainXPCallback, isMultiplayer
            ...
            if (dist < 20) {
                if (localPlayer.abilities.healOnXp && Math.random() < 0.1) { // 10% chance
                    localPlayer.health = Math.min(localPlayer.maxHealth, localPlayer.health + 1); // Heal 1 HP
                }
    ...
    if (gameState.gameTime - skill.lastStrike > skill.cooldown) {
        ...
        for (let i = 0; i <= skill.chains; i++) {
            ...
            potentialTargets.forEach(target => {
                ...
                if (target && dist < minDist && dist < 300) { // Ensure target is not null/undefined
                    minDist = dist;
            ...
            if (closestTarget) {
                ...
                potentialTargets = potentialTargets.filter(t => t.id !== closestTarget.id); // Filter by ID
            } else { break; }
        }
    ...
    if (gameState.gameTime - skill.lastEruption > skill.cooldown) {
        if (enemies.length > 0) {
            ...
            for (let i=0; i < eruptionCount; i++) {
                // Select a random target from the *current* list of enemies
                ...
                const volcanoEffect = {
                    ...
                    life: skill.burnDuration, hitEnemyIds: new Set(), // Store hit enemy IDs to prevent re-hitting
                    update(dt) {
                        this.life -= dt;
                        enemies.forEach(e => { // Iterate over current local enemies
                            ...
                            if (dist < this.radius) {
                                // Continuous damage application over time (per frame)
                                const damageThisFrame = (this.damage / (this.life + dt)) * dt; // Approximate per frame damage
                                
                                if (gameState.isHost) {
                                    // Host updates enemy health in DB for continuous burn
                                    enemiesRef.child(e.id).child('health').set(e.health - damageThisFrame);
                                } else {
                                    e.health -= damageThisFrame; // Local estimate for non-host
                                }
                                // Visual effect of damage number, maybe only for first hit or periodically
                                if (!this.hitEnemyIds.has(e.id) || Math.random() < 0.05 * (dt / 16)) { // Periodically show damage
                                    spawnDamageNumber(e.x, e.y, damageThisFrame, false);
                                }
                                this.hitEnemyIds.add(e.id); // Mark enemy as continually affected by this burn
                            }
                        });
                        return this.life <= 0;
                    }
    ...
    if (gameState.gameTime - skill.lastCast > skill.cooldown) {
        if (enemies.length > 0) {
            ...
            const blackHoleEffect = {
                ...
                pulledEnemyIds: new Set(), // Store IDs, not objects
                update(dt) {
                    this.life -= dt;
                    enemies.forEach(e => { // Iterate over current local enemies
                        if (!e) return; // Skip if enemy is null/undefined
                        ...
                        if (dist < this.radius && dist > 10) { // Keep enemies from collapsing to the center
                            ...
                            if (gameState.isHost) {
                                ...
                                if (!this.pulledEnemyIds.has(e.id)) { // First hit on entry or refresh
                                    this.pulledEnemyIds.add(e.id);
                            ...
                            this.pulledEnemyIds.delete(e.id); // Remove from pulled set if outside radius
                        }
                    });
                    return this.life <= 0;
                }
